...
/**
 * Για τη διασύνδεση με τα δεδομένα χρησιμοποιείται η κλάση DBManager, 
 * η οποία υλοποιεί το πρότυπο σχεδίασης Singleton (Singleton Pattern),
 * διασφαλίζοντας ότι υπάρχει μόνο ένα ενεργό σημείο πρόσβασης στη βάση
 * καθ' όλη τη διάρκεια λειτουργίας της εφαρμογής.
 */
public class DBManager {
    ...
    // Τοπική αναζήτηση στην ΒΔ 
    // Ψάχνουμε τόσο βάση τίτλου και περιεχομένου.
    public List < Article > searchLocalArticles(String keyword) {
        // Δημιουργία EntityManager() για να επικοινωνήσουμε με τη βάση.
        EntityManager em = getEntityManager();
        try {
            // Αναζήτηση στον τίτλο ή το περιεχομένου/snippet.
            // Χρησιμοποιούμε UPPER() για case-insensitive αναζήτηση.
            return em.createQuery(
                    "SELECT a FROM Article a WHERE " +
                    "UPPER(a.title) LIKE UPPER(:keyword) OR " +
                    "UPPER(a.snippet) LIKE UPPER(:keyword)",
                    Article.class)
                .setParameter("keyword", "%" + keyword + "%")
                .getResultList();
        } catch (Exception e) {
            System.err.println("Σφάλμα αναζήτησης άρθρων με keyword '" + keyword + "': " + e.getMessage());
            return new ArrayList < > ();
        } finally {
            em.close();
        }
    ...
    // 2 μέθοδοι. Καταγραφή Λέξης-Κλειδιού για Στατιστικά
